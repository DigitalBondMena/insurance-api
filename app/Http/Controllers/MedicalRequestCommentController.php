...
class MedicalRequestCommentController extends Controller
{
    public function index($id)
    {
        $comments = MedicalRequestComment::where('request_id', $id)->with(['user', 'reciver', 'request'])->get();
        return response()->json([
            'success' => true,
            'message' => 'Medical Request Comments Fetched Successfully',
            'data' => $comments
        ], 200);
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'request_id' => 'required|exists:medical_requests,id',
        ...
        if($request->hasFile('comment_file')){
            ...
            $file->move(public_path('uploads/medical_request_comments'), $fileName);
            $requestArray['comment_file'] = 'uploads/medical_request_comments/'.$fileName;
        }
        ...
        $comment = MedicalRequestComment::create($requestArray);

        return response()->json([
            'success' => true,
            'message' => 'Medical Request Comment Added Successfully',
            'data' => $comment
        ], 201);
    }
    ...
    {
        $comment = MedicalRequestComment::findorfail($id);
        return response()->json([
            'success' => true,
            'message' => 'Medical Request Comment Fetched Successfully',
            'data' => $comment
        ], 200);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->hasFile('comment_file')){
            ...
            if ($comment->comment_file) {
                $oldImage = public_path('uploads/medical_request_comments/' . $comment->comment_file);
        ...
        return response()->json([
            'success' => true,
            'message' => 'Medical Request Comment Updated Successfully',
            'data' => $comment
        ], 200);
