...
class JopRequestCommentController extends Controller
{
    public function index($id)
    {
        $comments = JopRequestComment::where('request_id', $id)->with(['user', 'reciver', 'request'])->get();
        return response()->json([
            'success' => true,
            'message' => 'Jop Request Comments Fetched Successfully',
            'data' => $comments
        ], 200);
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'request_id' => 'required|exists:jop_requests,id',
        ...
        if($request->hasFile('comment_file')){
            ...
            $file->move(public_path('uploads/jop_request_comments'), $fileName);
            $requestArray['comment_file'] = 'uploads/jop_request_comments/'.$fileName;
        }
        ...
        $comment = JopRequestComment::create($requestArray);

        return response()->json([
            'success' => true,
            'message' => 'Jop Request Comment Added Successfully',
            'data' => $comment
        ], 201);
    }
    ...
    {
        $comment = JopRequestComment::findorfail($id);
        return response()->json([
            'success' => true,
            'message' => 'Jop Request Comment Fetched Successfully',
            'data' => $comment
        ], 200);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->hasFile('comment_file')){
            ...
            if ($comment->comment_file) {
                $oldImage = public_path('uploads/jop_request_comments/' . $comment->comment_file);
        ...
        return response()->json([
            'success' => true,
            'message' => 'Jop Request Comment Updated Successfully',
            'data' => $comment
        ], 200);
